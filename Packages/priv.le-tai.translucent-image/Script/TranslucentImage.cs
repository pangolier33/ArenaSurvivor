...
public partial class TranslucentImage : Image, IMeshModifier
{
    ...
    private static Shader correctShader;
    ...
    void PrepareShader()
    {
        correctShader = correctShader ? correctShader : Shader.Find("UI/TranslucentImage");
    }
    ...
    void LateUpdate()
    {
        ...
        if (!material || material.shader.name != correctShader.name)
        {
            Debug.LogError($"[TranslucentImage] LateUpdate | require shader \"UI/TranslucentImage\" | used shader {material.shader?.name}");
        }
