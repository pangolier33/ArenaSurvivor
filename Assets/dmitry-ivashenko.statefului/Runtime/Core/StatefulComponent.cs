...
namespace StatefulUI.Runtime.Core
{
    public sealed partial class StatefulComponent : MonoBehaviour
    {
        ...
        {
	        if (Enum.TryParse(typeof(StateRole),role, out var value))
	        {
		        SetState(role);
		        return;
	        }
	        var go = gameObject;
	        Debug.LogError($"View {name} does not contain state with role {role}, scene path: {go.GetScenePath()}", go);
        }
        public float SetState(StateRole role) => SetState((int)role);
