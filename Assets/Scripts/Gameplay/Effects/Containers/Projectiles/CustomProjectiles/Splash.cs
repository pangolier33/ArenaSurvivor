...
{
	public class Splash : ProjectileEffectContainer
	{
		...
		protected override void OnSpawned()
		{
			...
			//side
			Vector2 side = Settings.Trace.GetVariable<Vector2>("side");
			side.x *= -1.0f;
			Settings.Trace.SetVariable("side", side);
			Vector3 scale = transform.localScale;
			scale.x = Mathf.Abs(scale.x) * side.x;
